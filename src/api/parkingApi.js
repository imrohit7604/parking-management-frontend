...
export const releaseVehicle = async (vehicle_Id) => {
  ...
  await axios
    .put(
      "http://localhost:3002/api/vehicleParking",
      { vehicle_Id },
      {
        headers: {
          "x-auth-token": jsonToken,
...
export const parkVehicle = async (
  parkingZoneId,
  parkingSpaceId,
  registrationNumber
) => {
  ...
  await axios
    .post(
      "http://localhost:3002/api/vehicleParking",
      { parkingZoneId, parkingSpaceId, registrationNumber },
      {
        headers: {
          "x-auth-token": jsonToken,
        },
...
export const getParkingDetails = async (date) => {
  ...
  await axios
    .post(
      "http://localhost:3002/api/vehicleParking/parkingDetails",
      {
        date: {
          day: date.getDate(),
          month: date.getMonth(),
