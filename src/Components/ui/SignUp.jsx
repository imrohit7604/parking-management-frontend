...
export default function SignUp() {
  ...
  const handelSubmit = async (event) => {
    ...
    if (isVaild) {
      const response = await signUp({
        ...
        typeOfUser: localState.typeOfUser === "true" ? true : false,
      });
      if (response.status === 200)
        setState({
          ...localState,
          signUpError: "",
          signUpSuccess: "Registred Successfully :)",
        });
      else
        setState({
          ...localState,
          signUpSuccess: "",
          signUpError: "Already Registred",
        });
  ...
  if (state.user) return <Redirect to={"/dashboard"} />;
  else {
    return (
