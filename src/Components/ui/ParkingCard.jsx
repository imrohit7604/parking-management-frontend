...
const ParkingCard = (props) => {
  ...
  const { releaseSpaceAndVehicle, getParkingSpaces, bookSpace } = useContext(
    ParkingContext
  );
  ...
  const handleSubmit = async (event) => {
    ...
    if (registrationNumber != null) {
      const response = await releaseSpaceAndVehicle(
        parkingSpaceTitle.vehicleId
      );
      ...
      if (vaildate()) {
        const response = await bookSpace(
          parkingZoneId,
          parkingSpaceId,
        ...
        if (response.status === 200) {
          ...
          setOpen(false);
        } else setState({ ...localState, error: true });
      }
    }
  };
  ...
  return (
    <>
      <Card className={classes.root}>
        <CardHeader
          className={
            registrationNumber ? classes.headOccupied : classes.headVacant
          }
          title={
        ...
        <CardContent>
          ...
          <CardActions style={{ justifyContent: "center" }}>
            <Button
              variant="contained"
              className={registrationNumber ? classes.button : null}
              color="primary"
              disabled={state.user && !state.user.typeOfUser}
              onClick={handleOpen}
            >
              {registrationNumber ? "Release" : "Book"}{" "}
            </Button>
          </CardActions>
        </CardContent>
      </Card>
        ...
        <Fade in={open}>
          <div className={classes.paper}>
            <h1 id="transition-modal-title">
              {registrationNumber
                ? "Do You want to release this vehicle"
