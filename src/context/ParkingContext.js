...
const resetDetails = (dispatch) => {
  return async () => {
    ...
    if (response.status === 200) dispatch({ type: "reset" });
...
const releaseSpaceAndVehicle = (dispatch) => {
  return async (vehicle_Id) => {
    ...
    if (response.status === 200) dispatch({ type: "vehicle_release" });
    else dispatch({ type: "add_error", payload: response.data.error });
    return response;
  };
};
const bookSpace = (dispatch) => {
  return async (parkingZoneId, parkingSpaceId, registrationNumber) => {
    ...
    if (response.status === 200) dispatch({ type: "book_space" });
    else dispatch({ type: "add_error", payload: response.data.error });
    return response;
  };
};
